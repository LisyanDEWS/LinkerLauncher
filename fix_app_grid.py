...
# We need to replace the damaged part.
# The damaged part starts around id="row1-bento-grid"
pattern = re.compile(r'<main className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="row1-bento-grid">.*?</button>\s*</div>\s*</div>\s*\{/\* PANEL: Link list with real working COPY buttons \*/\}', re.DOTALL)

replacement = r'''<main className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="row1-bento-grid">
