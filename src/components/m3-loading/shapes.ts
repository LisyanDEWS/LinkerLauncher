 ...
 *
 * SVG path data extracted from official Material Design shape assets.
...
const SVG_DEFS: (ShapeDef | null)[] = [
  ...
  // 6: OVAL — no SVG asset; generated as ellipse
  null,
];

// ---------------------------------------------------------------------------
// SVG Path → Point Array
// ---------------------------------------------------------------------------
...
function svgPathToPoints(d: string): Point[] {
  ...
  for (const { cmd, args } of cmds) {
    ...
    // Z/z: close path (no new points needed for sampling)
  }
  return pts;
}
...
// ---------------------------------------------------------------------------
// Build & export pre-computed shape arrays
// ---------------------------------------------------------------------------

let _shapes: Point[][] | null = null;

/** Returns all 7 shape point arrays (lazy-initialized). */
...
/** Get the morphed shape at a given morph fraction. */
export function getMorphedShape(morphFraction: number): Point[] {
