...
const extractWallpaperAnalysis = (imageUrl: string): Promise<WallpaperAnalysis> => {
  ...
  const openAppWithMode = (opts: Parameters<typeof wm.open>[0]) => {
    if (appMode === 'classic') {
      ...
      if (opts.id === 'proxy') {
        const proxyUrl = proxyInitialUrl || localStorage.getItem('linkerru_server_url') || 'https://math.soyescalahumana.cl/';
  ...
  if (standaloneParam === 'proxy') {
    const proxyUrl = localStorage.getItem('linkerru_server_url') || 'https://math.soyescalahumana.cl/';
    return (
  ...
  return (
    <>
      ...
      <section className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4" id="row2-bento-grid">
        ...
        {/* TELEGRAM ROUTE CARD (Restored active state) */}
        <div 
          onClick={() => {
            playChime('click');
            openTelegramRouteWindow();
          }}
          className="card panel-gradient rounded-3xl p-6 flex flex-col justify-between min-h-[250px] transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer relative" 
          id="card-telegram-route"
        >
          ...
          <div className="flex justify-between items-start h-[44px]">
            <div
              className="w-11 h-11 rounded-2xl border border-[var(--btn-border)] overflow-hidden flex items-center justify-center p-0 shadow-inner"
              style={{
                backgroundColor: theme === 'dark' ? 'var(--btn-bg)' : activePalette.primary,
              }}
            >
